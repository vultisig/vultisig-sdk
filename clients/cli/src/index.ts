...
async function init(): Promise<CLIContext> {
  if (!ctx) {
    ...
    if (existingVault) {
      await ctx.setActiveVault(existingVault)
  ...
  .action(
    withExit(async (chainStr: string | undefined, options: { tokens?: boolean }) => {
      ...
      await executeBalance(context, {
        chain: chainStr as Chain | undefined,
  ...
  .action(
    withExit(async (options: { add?: string; remove?: string }) => {
      ...
      await executeChains(context, {
        add: options.add as Chain | undefined,
        remove: options.remove as Chain | undefined,
      })
    })
  )
    ...
    withExit(
      async (
        ...
        await executeSwapQuote(context, {
          fromChain: fromChainStr as Chain,
          toChain: toChainStr as Chain,
