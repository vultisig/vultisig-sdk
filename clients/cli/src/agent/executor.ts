...
export class AgentExecutor {
  ...
  private backendClient: {
    getCalldata(
      id: string,
    ): Promise<{ data: string; to?: string; chain?: string }>;
  } | null = null;
  ...
  storeServerTransaction(txReadyData: any): boolean {
    ...
    const nestedTx =
      txReadyData?.swap_tx || txReadyData?.send_tx || txReadyData?.tx;
    if (nestedTx?.status === "error" || nestedTx?.error) {
      if (this.verbose)
        process.stderr.write(
          `[executor] skipping error tx_ready: ${nestedTx.error || "unknown error"}\n`,
        );
      return false;
    }
    if (!nestedTx) {
      if (this.verbose)
        process.stderr.write(
          `[executor] storeServerTransaction: no swap_tx/send_tx/tx found in data\n`,
        );
      return false;
    }
  ...
  shouldAutoExecute(action: Action): boolean {
    return (
      action.auto_execute === true || AUTO_EXECUTE_ACTIONS.has(action.type)
    );
  }
  ...
  private async dispatch(action: Action): Promise<Record<string, unknown>> {
    if (this.verbose)
      process.stderr.write(
        `[dispatch] action.type=${action.type} action.id=${action.id}\n`,
      );
  ...
  private async getBalances(
    ...
    let entries = Object.entries(balanceRecord).map(
      ([key, b]: [string, any]) => ({
    ...
    if (chainFilter) {
      ...
      if (chain) {
        entries = entries.filter(
          (b) => b.chain.toLowerCase() === chain.toLowerCase(),
        );
      }
    }
    ...
    if (tickerFilter) {
      entries = entries.filter(
        (b) => b.symbol.toLowerCase() === tickerFilter.toLowerCase(),
      );
    }
  ...
  private async getPortfolio(
    ...
    const fiatCurrency: FiatCurrency = (
      fiatCurrencies as readonly string[]
    ...
    if (tickerFilter) {
      rows = rows.filter(
        (r) => r.symbol.toLowerCase() === String(tickerFilter).toLowerCase(),
      );
    }
  ...
  private async removeCoin(
    ...
    const tokenId = (params.token_id ||
      params.id ||
      params.contract_address) as string;
  ...
  private async buildSendTx(
    ...
    const symbol = (params.symbol || params.ticker) as string;
    const toAddress = (params.address ||
      params.to ||
    ...
    try {
      ...
      const balance = await this.vault.balance(
        chain,
        params.token_id as string | undefined,
      );
  ...
  private async buildSwapTx(
    ...
    if (this.verbose)
      process.stderr.write(
        `[build_swap_tx] called with params: ${JSON.stringify(params).slice(0, 500)}\n`,
      );
    ...
    try {
      const amountStr = params.amount as string;
      const fromSymbol = (params.from_symbol ||
        params.from_token ||
        "") as string;
      const toSymbol = (params.to_symbol || params.to_token || "") as string;
      const fromToken = (params.from_contract || params.from_token_id) as
        | string
        | undefined;
      const toToken = (params.to_contract || params.to_token_id) as
        | string
        | undefined;
  ...
  private async buildTx(
    ...
    if (params.calldata_id && !params.data && this.backendClient) {
      ...
      if (this.verbose)
        process.stderr.write(`[executor] resolving calldata_id ${id}\n`);
      ...
      if (this.verbose)
        process.stderr.write(
          `[executor] calldata_id resolved, data len=${entry.data.length}\n`,
        );
    }
    ...
    if (params.data || params.calldata || params.hex_payload) {
      ...
      if (!stored) {
        throw new Error(
          "Could not stage calldata transaction for signing (invalid or empty tx payload)",
        );
      }
  ...
  private async buildContractCallTx(
    ...
    const typedParams = params.params as
  ...
  private async signTx(
    ...
    if (this.verbose)
      process.stderr.write(
        `[sign_tx] params: ${JSON.stringify(params).slice(0, 500)}\n`,
      );
    if (this.verbose)
      process.stderr.write(
        `[sign_tx] pendingPayloads keys: ${[...this.pendingPayloads.keys()].join(", ")}\n`,
      );

    // Find the pending payload
    const payloadId = (params.keysign_payload ||
      params.payload_id ||
      "latest") as string;
    ...
    if (!stored) {
      throw new Error(
        "No pending transaction to sign. Build a transaction first.",
      );
    }
    ...
    if (payload.__serverTx) {
      ...
      if (
        ...
        (payload.swap_tx || payload.provider)
      ) {
        try {
  ...
  private async signSdkTx(
    ...
    try {
      ...
      // Record broadcast timestamp for nonce grace period (prevents stale-state reset on back-to-back txs)
  ...
  private async signServerTx(
    ...
    const swapTx =
      serverTxData.swap_tx || serverTxData.send_tx || serverTxData.tx;
    if (!swapTx?.to) {
    ...
    const chainName = (params.chain ||
      serverTxData.chain ||
      serverTxData.from_chain) as string | undefined;
    const chainId = (serverTxData.chain_id || swapTx.chainId) as
      | string
      | number
      | undefined;
    ...
    try {
      ...
      if (swapTx.gas_limit) {
        ...
        if (bs?.case === "ethereumSpecific" && bs.value?.gasLimit) {
          ...
          if (BigInt(serverGas) > BigInt(currentGas)) {
            ...
            if (this.verbose)
              process.stderr.write(
                `[gas] Using server gas_limit: ${serverGas} (was ${currentGas})\n`,
              );
          }
        }
      }
      ...
      // Extract message hashes and sign
      const messageHashes =
        await this.vault.extractMessageHashes(keysignPayload);
      ...
      try {
        this.recordEvmNonceFromPayload(
          chain,
          keysignPayload,
  ...
  private async buildAndSignSolanaSwapLocally(
    ...
    if (serverTxData._phase === "prepare") {
      throw Object.assign(
        new Error("tx_ready prepare phase: deferring to server sign path"),
        {
          _phase: "prepare",
        },
      );
    ...
    const fromChainName =
      serverTxData.from_chain || serverTxData.chain || "Solana";
    ...
    if (!amountStr)
      throw Object.assign(
        new Error(
          "Missing amount in tx_ready data for local Solana swap build",
        ),
        {
          _phase: "prepare",
        },
    ...
    if (fromDecimals == null)
      throw Object.assign(
        new Error(
          "Missing from_decimals in tx_ready data for local Solana swap build",
        ),
        {
          _phase: "prepare",
        },
    ...
    try {
      ...
      throw Object.assign(
        new Error(
          `Invalid amount in tx_ready data for local Solana swap build: ${amountStr}`,
        ),
        {
          _phase: "prepare",
        },
  ...
  private async acquireEvmLockIfNeeded(chain: Chain): Promise<void> {
    ...
    if (this.verbose)
      process.stderr.write(`[nonce] Acquired lock for ${chain}\n`);
  }
  ...
  private async releaseEvmLock(chain: Chain): Promise<void> {
    ...
    if (release) {
      ...
      if (this.verbose)
        process.stderr.write(`[nonce] Released lock for ${chain}\n`);
    }
  }
  ...
  private async patchEvmNonce(chain: Chain, payload: any): Promise<void> {
    ...
    if (nextNonce !== rpcNonce) {
      // Grace period: if we broadcast recently, the previous tx may still be propagating
      // to the mempool. Don't reset nonce backwards — trust local state.
      const lastBroadcast = this.evmLastBroadcast.get(chain.toString()) ?? 0;
      const elapsed = Date.now() - lastBroadcast;
      if (elapsed < 15_000) {
        if (this.verbose)
          process.stderr.write(
            `[nonce] Keeping local nonce ${nextNonce} for ${chain} (broadcast ${elapsed}ms ago, within 15s grace period)\n`,
          );
        bs.value.nonce = nextNonce;
        if (this.verbose)
          process.stderr.write(
            `[nonce] Patched ${chain} nonce: ${rpcNonce} → ${nextNonce}\n`,
          );
        return;
      }
  ...
  private async patchEvmGas(chain: Chain, payload: any): Promise<void> {
    ...
    try {
      ...
      if (this.verbose)
        process.stderr.write(
          `[gas] Failed to refresh base fee for ${chain}, keeping original\n`,
        );
    }
  }
   ...
   */
  private recordEvmNonceFromPayload(
    chain: Chain,
    payload: any,
    numTxs: number,
  ): void {
    ...
    if (this.verbose)
      process.stderr.write(
        `[nonce] Recorded ${chain} nonce: ${highestNonce}\n`,
      );
  }
  ...
  private async signTypedData(
    ...
    const payloads = params.payloads as
      | Array<Record<string, unknown>>
      | undefined;
    if (payloads && Array.isArray(payloads)) {
      if (this.verbose)
        process.stderr.write(
          `[sign_typed_data] payloads mode, ${payloads.length} items\n`,
        );
      ...
      for (let i = 0; i < payloads.length; i++) {
        ...
        if (i > 0) {
          if (this.verbose)
            process.stderr.write(
              `[sign_typed_data] waiting 5s between MPC sessions...\n`,
            );
   ...
   */
  private async signSingleTypedData(
    params: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
    const domain = params.domain as Record<string, unknown>;
    const types = params.types as Record<
      string,
    ...
    if (!domain || !types || !message || !primaryType) {
      throw new Error(
        "sign_typed_data requires domain, types, message, and primaryType",
      );
    }

    if (this.verbose)
      process.stderr.write(
        `[sign_typed_data] primaryType=${primaryType} domain.name=${domain.name}\n`,
      );

    const eip712Hash = await computeEIP712Hash(
      domain,
      types,
      primaryType,
      message,
    ...
    if (this.verbose)
      process.stderr.write(
        `[sign_typed_data] signed, format=${sigResult.format}, recovery=${sigResult.recovery}\n`,
      );
    ...
    if (this.verbose)
      process.stderr.write(
        `[sign_typed_data] r=${r.slice(0, 16)}... s=${s.slice(0, 16)}... v=${v}\n`,
      );
  ...
  private async getAddressBook(
    ...
    const chainName =
      (params.chain as string | undefined) ||
      (params.chain_name as string | undefined);
    const chain = chainName ? resolveChain(chainName) : undefined;
    if (chainName && !chain) {
      throw new Error(`Unknown chain: ${chainName}`);
    }

    return (await this.vultisig.getAddressBook(chain)) as unknown as Record<
      string,
      unknown
    >;
  }

  private async addAddressBookEntry(
  ...
  private async removeAddressBookEntry(
    _params: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
    throw new Error(
  ...
  private async searchToken(
    ...
    const tokenMatchesQuery = (t: {
      ...
      return (
        tick.includes(query) || addr.includes(query) || pid.includes(query)
      );
    };
    ...
    if (chainName) {
      const chain = resolveChain(chainName);
      if (!chain) throw new Error(`Unknown chain: ${chainName}`);
      const tokens = VultisigSdk.getKnownTokens(chain)
        .filter(tokenMatchesQuery)
        .slice(0, limit);
      return { tokens };
    }
  ...
  private async scanTx(
    _params: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
    throw new Error(
      "scan_tx is not yet implemented locally. The backend may handle this action server-side.",
    );
  }

  private async readEvmContract(
    params: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
    const chainName = params.chain as string | undefined;
    ...
    const contractRaw =
      (params.contract_address as string | undefined) ||
      (params.contractAddress as string | undefined);
    if (!contractRaw)
      throw new Error("read_evm_contract requires contract_address");

    const functionName =
      (params.function_name as string | undefined) ||
      (params.functionName as string | undefined);
    if (!functionName)
      throw new Error("read_evm_contract requires function_name");

    const chain = resolveChain(chainName);
    if (!chain) throw new Error(`Unknown chain: ${chainName}`);
    if (!EVM_CHAINS.has(chain)) {
      throw new Error(
        `read_evm_contract only supports EVM chains (got ${chain})`,
      );
    }

    const callParams =
      (params.params as Array<{ type: string; value: string }> | undefined) ??
      [];
    const data = (await encodeContractCall(
      functionName,
      callParams,
    )) as `0x${string}`;

    const addr = contractRaw.startsWith("0x")
      ? contractRaw
      : `0x${contractRaw}`;
...
function encodeField(
  ...
  if (type.endsWith("]")) {
    ...
    const encodedElements = arr.map((el) =>
      encodeField(elementType, el, types, keccak),
    );
...
function parseDERSignature(sigHex: string): { r: string; s: string } {
  ...
  if (raw.slice(offset, offset + 2) !== "02")
    throw new Error("Invalid DER: expected 02 for R");
  offset += 2;
  ...
  // S value
  if (raw.slice(offset, offset + 2) !== "02")
    throw new Error("Invalid DER: expected 02 for S");
  offset += 2;
