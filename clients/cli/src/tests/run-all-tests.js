  ...
  return new Promise((resolve) => {
    ...
    testProcess.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        resolve({ name: testName, passed: false, duration, output: stderr || stdout });
      }
    });
    ...
    setTimeout(() => {
      ...
      resolve({ name: testName, passed: false, duration: 60000, output: 'Test timed out' });
    }, 60000);
  });
}
...
async function runCLICommand(command, args = []) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      resolve({ code: -1, stdout, stderr: 'Command timed out', duration: 30000 });
    }, 30000);
  });
}
