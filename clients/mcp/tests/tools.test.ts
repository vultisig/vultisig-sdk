...
import { getToolNames, getTools } from '../src/tools'
...
describe('tool registration', () => {
  ...
  it('registers all 8 tools with full profile', () => {
    const vault = createMockVault()
    const tools = getTools(vault, 'full')
...
describe('profile filtering', () => {
  ...
  it('full profile matches defi', () => {
    const vault = createMockVault()
    const defiTools = Object.keys(getTools(vault, 'defi'))
    const fullTools = Object.keys(getTools(vault, 'full'))
    expect(fullTools).toEqual(defiTools)
  })
  ...
  it('getToolNames returns correct names per profile', () => {
    ...
    expect(getToolNames('full')).toHaveLength(8)
