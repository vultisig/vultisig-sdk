...
const CHAINS = [
  ...
  'polygon', 'binance-smart-chain', 'avalanche', 'arbitrum', 'optimism',
  'cosmos', 'osmosis', 'litecoin', 'dogecoin', 'bitcoin-cash',
  // ... all 30+ chains
];
...
export const mockServer = setupServer(
  ...
  rest.get(`${API_BASE}/router/:sessionId`, (req, res, ctx) => {
    return res(
      ctx.json(['participant_1', 'participant_2'])
    );
  })
);
...
// package.json additions
{
  ...
  "lint-staged": {
    ...
    "tests/fixtures/**/*.json": [
      "node tests/scripts/validate-fixtures.js"
    ]
  },
  "devDependencies": {
    "husky": "^8.0.0",
...
*Phase 1 establishes the critical testing foundation. Successful completion enables rapid test development in subsequent phases.*