...
class Vault {
  ...
  async updateBalances(
    chains?: string[],
    includeTokens?: boolean,
  ): Promise<Record<string, Record<string, Balance>>>; // Force refresh balances
  ...
  // === VAULT OPERATIONS ===
  async reshare(options: {
    newParticipants: string[];
    removeParticipants?: string[];
  }): Promise<Vault>; // Change vault participants
