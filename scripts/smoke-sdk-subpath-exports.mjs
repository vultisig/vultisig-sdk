...
try {
  ...
  writeFileSync(
    ...
    [
      ...
      "const serverPath = require.resolve('@vultisig/sdk/server')",
      ...
      'assert.match(serverPath, /dist\\/server\\/index\\.cjs$/)',
      ...
      "const serverModule = await import('@vultisig/sdk/server')",
      ...
      "assert.equal(typeof serverModule.setupVaultWithServer, 'function')",
      "assert.equal(typeof serverModule.signWithServer, 'function')",
      "assert.equal(typeof serverModule.sendMpcRelayMessage, 'function')",
      'console.log(JSON.stringify({ parsePath, defiPath, bridgePath, tronPath, utxoPath, decodePath, txPath, serverPath, parseOk: true, defiOk: true, bridgeOk: true, tronOk: true, utxoOk: true, decodeOk: true, txOk: true, serverOk: true }))',
      '',
    ].join('\n')
  )
  writeFileSync(
    ...
    [
      ...
      "import { setupVaultWithServer, type MpcRelayMessage } from '@vultisig/sdk/server'",
      '',
      ...
      'void normalized',
      'const serverSetup: typeof setupVaultWithServer = setupVaultWithServer',
      'void serverSetup',
      'const relayMessage = null as unknown as MpcRelayMessage',
      'void relayMessage',
      '',
    ].join('\n')
  )
