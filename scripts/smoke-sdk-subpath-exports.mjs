...
try {
  ...
  writeFileSync(
    ...
    [
      ...
      "assert.equal(typeof policyModule.policy, 'object')",
      "assert.equal(typeof policyModule.evaluatePolicy, 'function')",
      ...
      'console.log(JSON.stringify({ parsePath, defiPath, bridgePath, policyPath, gasPath, balancePath, tronPath, utxoPath, decodePath, txPath, parseOk: true, defiOk: true, bridgeOk: true, policyOk: true, gasOk: true, balanceOk: true, tronOk: true, utxoOk: true, decodeOk: true, txOk: true }))',
      '',
    ].join('\n')
  )
  writeFileSync(
    ...
    [
      ...
      'void chainConfig',
      'const evaluator: typeof policy.evaluate = policy.evaluate',
      'void evaluator',
