...
function decodeVault(buffer: Buffer): VaultInfo {
  ...
  return {
    ...
    createdAt: vaultProtobuf.createdAt
      ? new Date(Number(vaultProtobuf.createdAt.seconds) * 1000)
      : null,
...
function formatVaultInfo(
  ...
  if (vault.keyShares && vault.keyShares.length > 0) {
    vault.keyShares.forEach((keyShare, index) => {
      ...
      info.push(
        `  Key Share: ${keyShare.keyshare ? `${keyShare.keyshare.substring(0, 50)}...` : "N/A"}`,
      );
      info.push("");
    });
  } else {
  ...
  info.push(
    `Total Key Shares: ${vault.keyShares ? vault.keyShares.length : 0}`,
  );
  info.push(
    `Security Type: ${vault.signers && vault.signers.length === 2 ? "Fast (2-of-2)" : "Secure (Multi-party)"}`,
  );
  info.push("");
...
function main(): void {
  ...
  if (args.length === 0) {
    console.error(
      "Usage: npx tsx scripts/inspect_keyshare.ts <vault.vult> [password]",
    );
    console.error("");
    console.error("Examples:");
    console.error(
      "  npx tsx scripts/inspect_keyshare.ts vault.vult                # Unencrypted vault",
    );
    console.error(
      "  npx tsx scripts/inspect_keyshare.ts vault.vult mypassword123  # Encrypted vault",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    // Parse VaultContainer protobuf using proper schema
    ...
    if (container.isEncrypted) {
      if (!password) {
        console.error(
          "Error: This vault is encrypted. Please provide a password.",
        );
        console.error(
          `Usage: npx tsx scripts/inspect_keyshare.ts ${vaultFile} <password>`,
        );
        process.exit(1);
      }
      ...
      try {
        ...
        console.error(
          "Error: Failed to decrypt vault. Invalid password or corrupted data.",
        );
      ...
      if (password) {
        console.warn(
          "Warning: Password provided but vault is not encrypted. Ignoring password.",
        );
      }
      vaultBuffer = fromBase64(container.vault);
    }

    // Parse the inner Vault protobuf using proper schema
