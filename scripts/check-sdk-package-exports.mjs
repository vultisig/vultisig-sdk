...
function validatePackedReactNativeTokenRefExports(packageRoot) {
  ...
  for (const name of ['resolveTokenRef', 'resolveTokenRefId', 'amount']) {
    assert.ok(exportedNames.includes(name), `packed React Native runtime must export ${name}`)
  }
  console.log(
    'SDK packed React Native token resolver and amount export bindings passed (artifact check, not device execution)'
  )
}
