...
const lockfilePath = resolve(
  repoRoot,
  getArgValue("--lockfile") ?? "yarn.lock",
);
const metadataFile =
...
const packageNameFromDescriptor = (descriptor) => {
  ...
  const match = normalized.match(
    /^(@[^/]+\/[^@]+|[^@]+)@(?:npm:|patch:|workspace:|file:|portal:|link:|exec:|git:|https?:|[^@]+$)/,
  );
  ...
  for (const pattern of patterns) {
    ...
    for (const entry of entries) {
      ...
      if (statSync(dir).isDirectory()) {
        manifests.add(
          join(dir, pattern.slice(wildcardIndex + 1), "package.json"),
        );
      }
    }
  }
...
const collectDirectDescriptors = () => {
  ...
  for (const manifestPath of workspaceManifestPaths(rootPackage)) {
    ...
    for (const [selector, range] of Object.entries(
...
const parseYarnLock = () => {
  ...
  for (const line of readFileSync(lockfilePath, "utf8").split(/\r?\n/)) {
    ...
    const resolution = line.match(/^ {2}resolution: "?([^"]+)"?$/);
  ...
  return entries
    .map((entry) => {
      ...
      return {
        ...
        descriptors: entry.descriptorText
          .split(/,\s+/)
...
const metadataFixture = metadataFile
  ? readJson(resolve(repoRoot, metadataFile))
  : undefined;
...
const main = async () => {
  ...
  const directResolved = parseYarnLock()
    .filter((entry) =>
      entry.descriptors.some((descriptor) => directDescriptors.has(descriptor)),
    )
    .sort(
      (a, b) =>
