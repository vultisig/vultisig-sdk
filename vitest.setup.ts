...
global.fetch = async (url: string | URL | Request) => {
  ...
  if (urlString.includes('.wasm')) {
    ...
    if (urlString.includes('wallet-core.wasm')) {
      ...
      const possiblePaths = [
        path.join(__dirname, 'node_modules/@trustwallet/wallet-core/dist/lib/', filename),
        path.join(__dirname, 'lib/dkls/', filename),
        path.join(__dirname, 'lib/schnorr/', filename),
        path.join(__dirname, 'src/node_modules/@trustwallet/wallet-core/dist/lib/', filename)
      ]
    ...
    try {
      ...
      const fallbackPaths = [
        ...
        join(__dirname, 'node_modules/@trustwallet/wallet-core/dist/lib/wallet-core.wasm')
      ]
...
console.log('Vitest setup: Global WASM loading configured for Node.js environment')
