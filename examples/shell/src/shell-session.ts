...
export class ReplSession {
  ...
  private completeVaultName(partial: string): [string[], string] {
    const vaultNames = Array.from(this.vaults.values()).map(
      (vault) => vault.name,
    );
    const partialLower = partial.toLowerCase();
    const matches = vaultNames.filter((name: string) =>
  ...
  private findVaultByName(name: string): VaultBase | null {
    ...
    const found = vaults.find(
      (vault) => vault.name.toLowerCase() === nameLower,
    );
    return found || null;
  }
  ...
  private async cmdImport(args: string[]): Promise<void> {
    ...
    const vault = await this.sdk.importVault(
      vultContent,
      password || undefined,
    );
