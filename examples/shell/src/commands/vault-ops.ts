...
export function displayUnlocked(timeRemainingFormatted: string): void {
  console.log(chalk.green(`✓ Vault unlocked (valid for ${timeRemainingFormatted})`))
}
...
export function displayStatus(status: VaultStatus): void {
  ...
  console.log(`  Status:        ${status.isUnlocked ? chalk.green('Unlocked 🔓') : chalk.yellow('Locked 🔒')}`)
  if (status.isUnlocked && status.timeRemainingFormatted) {
    console.log(`  TTL:           ${chalk.blue(status.timeRemainingFormatted)} remaining`)
  }
