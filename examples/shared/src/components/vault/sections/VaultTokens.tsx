...
export default function VaultTokens({ vault }: VaultTokensProps) {
  ...
  const [selectedChain, setSelectedChain] = useState<string>(vault.chains[0] || '')
  ...
  const handleAddToken = async () => {
    ...
    try {
      ...
      await sdk.addToken(vault.id, selectedChain, token)

      // Update local state
      setTokensByChain(prev => ({
        ...prev,
        [selectedChain]: [...(prev[selectedChain] || []), token],
      }))
  ...
  const handleRemoveToken = async (chain: string, tokenId: string) => {
    ...
    try {
      await sdk.removeToken(vault.id, chain, tokenId)

      // Update local state
      setTokensByChain(prev => ({
        ...prev,
        [chain]: prev[chain]?.filter(t => t.id !== tokenId) || [],
      }))
    } catch (err) {
              ...
              <div className="divide-y divide-gray-100">
                {tokens.map(token => (
                  <TokenRow
                    key={token.id}
                    token={token}
                    onRemove={() => handleRemoveToken(chain, token.id)}
                    isLoading={isLoading}
                  />
                ))}
              </div>
...
// Token row component
function TokenRow({ token, onRemove, isLoading }: { token: TokenInfo; onRemove: () => void; isLoading: boolean }) {
  return (
