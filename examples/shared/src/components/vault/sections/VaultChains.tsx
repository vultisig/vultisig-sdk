...
export default function VaultChains({ vault, onVaultUpdated }: VaultChainsProps) {
  ...
  useEffect(() => {
    const loadChains = async () => {
      try {
        ...
        console.error('Failed to load chain list:', err)
      } finally {
  ...
  const availableChains = allChains.filter(chain => !vault.chains.includes(chain))
  ...
  const handleAddChain = async () => {
    ...
    try {
      await sdk.addChain(vault.id, selectedChain)
      // Update vault with new chain
      const updatedVault = {
        ...vault,
        chains: [...vault.chains, selectedChain],
      }
  ...
  const handleRemoveChain = async (chain: string) => {
    ...
    try {
      await sdk.removeChain(vault.id, chain)
      // Update vault with chain removed
      const updatedVault = {
        ...vault,
        chains: vault.chains.filter(c => c !== chain),
      }
      onVaultUpdated?.(updatedVault)
    } catch (err) {
...
function ChainCard({ chain, onRemove, isLoading }: { chain: string; onRemove: () => void; isLoading: boolean }) {
...
function getChainInfo(chain: string): { type: string; color: string } {
  // EVM chains
  const evmChains = [
    'Ethereum',
    'Polygon',
