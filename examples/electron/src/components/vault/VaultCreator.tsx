...
export default function VaultCreator({ onVaultCreated }: VaultCreatorProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create fast vault via IPC
      const result = await vault.createFast({
        name: formData.name,
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Verify vault via IPC
      const verifiedVault = await vault.verify(vaultId, verificationCode)
      onVaultCreated(verifiedVault)
      handleClose()
    } catch (err) {
