...
type VaultChainsProps = {
  vault: VaultInfo
  onChainsChanged?: () => void
}

// All available chains (subset for demo)
const ALL_CHAINS = [
  'Bitcoin',
  'Ethereum',
  'Polygon',
...
export default function VaultChains({ vault: vaultInfo, onChainsChanged }: VaultChainsProps) {
  ...
  const availableChains = ALL_CHAINS.filter(chain => !vaultInfo.chains.includes(chain))
  ...
  const handleAddChain = async () => {
    ...
    try {
      await vault.addChain(vaultInfo.id, selectedChain)
      setIsAddOpen(false)
      setSelectedChain('')
      onChainsChanged?.()
    } catch (err) {
  ...
  const handleRemoveChain = async (chain: string) => {
    ...
    try {
      await vault.removeChain(vaultInfo.id, chain)
      onChainsChanged?.()
    } catch (err) {
