...
function App() {
    ...
    appState.openVaults.forEach((vault) => {
      ...
      const handleBalanceUpdated = ({ chain }: any) => {
        addEvent(
          "balance",
          "vault",
          `${vaultPrefix} Balance updated for ${chain}`,
        );
      };
      ...
      const handleTransactionSigned = () => {
        addEvent(
          "success",
          "vault",
          `${vaultPrefix} Transaction signed successfully`,
        );
      ...
      const handleTransactionBroadcast = ({ chain, txHash }: any) => {
        addEvent(
          "transaction",
          "vault",
          `${vaultPrefix} Transaction broadcast on ${chain}: ${txHash}`,
        );
      ...
      const handleSigningProgress = ({ step }: any) => {
        addEvent(
          "signing",
          "vault",
          `${vaultPrefix} ${step.message} (${step.progress}%)`,
        );
      };
      ...
      const handleVaultError = (error: Error) => {
        addEvent(
          "error",
          "vault",
          `${vaultPrefix} Vault error: ${error.message}`,
        );
        showToast(error.message, "error");
      };
  ...
  const addEvent = (
    ...
    setAppState((prev) => ({
      ...prev,
      events: [...prev.events, createEvent(type, source, message, data)].slice(
        -1000,
      ), // Keep last 1000
    }));
  };
  ...
  const handleTabClose = async (vaultId: string) => {
    ...
    if (activeVault?.id === vaultId) {
      const remainingVaults = Array.from(appState.openVaults.values()).filter(
        (v) => v.id !== vaultId,
      );
      if (remainingVaults.length > 0) {
        await appState.sdk.setActiveVault(
          remainingVaults[remainingVaults.length - 1],
        );
      } else {
  ...
  return (
    <>
      <Layout
        ...
        eventLog={
          <EventLog events={appState.events} onClear={handleClearEvents} />
        }
      />
      {toast && <Toast {...toast} />}
    </>
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center text-gray-500">
          ...
          <p>
            Create a new vault, import one, or open an existing vault from the
            list.
          </p>
        </div>
      </div>
