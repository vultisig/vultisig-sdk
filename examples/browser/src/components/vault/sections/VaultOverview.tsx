...
export default function VaultOverview({ vault, sdk, onVaultDeleted, onVaultRenamed }: VaultOverviewProps) {
  ...
  const handleRename = async () => {
    ...
    try {
      await vault.rename(newName.trim())
  ...
  const handleExport = async () => {
    ...
    try {
      const { filename, data } = await vault.export(exportPassword)
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <InfoItem label="Encrypted" value={vault.isEncrypted ? 'Yes' : 'No'} />
          <InfoItem label="Chains" value={vault.chains.length.toString()} />
          <InfoItem label="Backed Up" value={vault.isBackedUp ? 'Yes' : 'No'} warning={!vault.isBackedUp} />
        </div>
      </div>
