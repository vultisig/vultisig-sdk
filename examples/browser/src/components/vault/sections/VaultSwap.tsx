...
export default function VaultSwap({ vault }: VaultSwapProps) {
  ...
  // Get tokens for selected chains (used for coin info lookup)
  const fromTokens = formData.fromChain ? vault.getTokens(formData.fromChain) : []
  const toTokens = formData.toChain ? vault.getTokens(formData.toChain) : []
  ...
  const handleGetQuote = async () => {
    ...
    try {
      ...
      const isSupported = await vault.isSwapSupported(formData.fromChain, formData.toChain)
      if (!isSupported) {
      ...
      // Get token info
      const fromToken = formData.fromTokenId ? fromTokens.find((t: Token) => t.id === formData.fromTokenId) : undefined
      const toToken = formData.toTokenId ? toTokens.find((t: Token) => t.id === formData.toTokenId) : undefined
      ...
      const currency = (vault.currency?.toLowerCase() || 'usd') as FiatCurrency
      const quoteResult = await vault.getSwapQuote({
        fromCoin,
        toCoin,
        amount,
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      // Use the stored quote or get a fresh one
      const currency = (vault.currency?.toLowerCase() || 'usd') as FiatCurrency
      const swapQuote =
        quote?.raw ||
      ...
      const swapResult = await vault.prepareSwapTx({
        ...
        amount,
        swapQuote,
        autoApprove: false, // Handle approval separately for UI feedback
      })
