...
export async function executeAuthSetup(
  ...
  if (!vaultFilePath) throw new Error('No vault file selected')
  ...
  try {
    ...
    if (!vault) throw new Error('unreachable: vault must be set after decrypt loop')
    const v = vault
    ...
    return { vaultId: v.id, vaultName: v.name, storageBackend: isUsingFileFallback() ? 'file' : ('keyring' as const) }
  } finally {
