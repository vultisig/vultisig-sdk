/**
 * E2E Test: SecureVault Creation from Seedphrase (3 SDK Instances)
 *
 * Tests programmatic creation of a 2-of-3 SecureVault by:
 * 1. SDK 1 (initiator) calls createSecureVaultFromSeedphrase() and generates QR
 * 2. SDK 2 & 3 (joiners) call joinSecureVault() with the QR payload
 * 3. All 3 SDKs coordinate via relay server to create vault shares
 * 4. Each SDK returns a SecureVault with working balance(), address(), etc.
 *
 * This test demonstrates the full multi-device SDK workflow without QR code scanning.
...
describe('E2E: SecureVault from Seedphrase (3 SDK Instances)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!TEST_CONFIG.seedphrase) {
      console.log('='.repeat(60))
      console.log('SKIPPING: TEST_SEEDPHRASE not set in .env file')
      console.log('')
      console.log('To run this test:')
      console.log('1. Create packages/sdk/tests/e2e/.env')
      console.log('2. Add: TEST_SEEDPHRASE=your mnemonic words here')
      console.log('')
      console.log('WARNING: Use a test seedphrase with small ETH balance!')
      console.log('='.repeat(60))
      return
    }

    console.log('='.repeat(60))
    console.log('SecureVault 3-SDK Import E2E Test')
    console.log('='.repeat(60))
  ...
  describe('Multi-Device Key Import', () => {
    it('should skip if seedphrase not configured', () => {
      if (!TEST_CONFIG.seedphrase) {
        console.log('Skipped: TEST_SEEDPHRASE not set')
        return
      }
  ...
  describe('Balance Detection', () => {
    ...
    it('should detect ETH balance on all 3 vaults', async () => {
      if (!vault1 || !vault2 || !vault3) return

      console.log('\n3. Checking ETH balance on all 3 vaults...')

      const [balance1, balance2, balance3] = await Promise.all([
        vault1.balance(Chain.Ethereum),
        vault2.balance(Chain.Ethereum),
        vault3.balance(Chain.Ethereum),
      ])

      // All vaults should have the same balance (same address)
      expect(balance1.amount).toBe(balance2.amount)
      expect(balance2.amount).toBe(balance3.amount)
      expect(balance1.symbol).toBe('ETH')

      console.log(`   ETH Balance: ${balance1.amount} ${balance1.symbol}`)
      console.log(`   All 3 vaults report identical balance`)
    }, 30000)

    it('should derive identical ETH address on all 3 vaults', async () => {
      if (!vault1 || !vault2 || !vault3) return

      console.log('\n4. Checking ETH address on all 3 vaults...')
    ...
    it('should detect THORChain balance', async () => {
      ...
      const balance = await vault1.balance(Chain.THORChain)

      expect(balance).toBeDefined()
      expect(balance.symbol).toBe('RUNE')
      console.log(`   RUNE Balance: ${balance.amount} ${balance.symbol}`)
    }, 30000)
    ...
    it('should detect Bitcoin balance', async () => {
      ...
      const balance = await vault1.balance(Chain.Bitcoin)

      expect(balance).toBeDefined()
      expect(balance.symbol).toBe('BTC')
      console.log(`   BTC Balance: ${balance.amount} ${balance.symbol}`)
    }, 30000)
  })
  ...
  describe('Cleanup', () => {
    it('should summarize test results', () => {
      console.log('\n' + '='.repeat(60))
      if (vault1 && vault2 && vault3) {
        console.log('3-SDK SecureVault Import Test PASSED!')
        console.log('')
        console.log('Summary:')
        console.log('- 3 SDK instances successfully coordinated')
        console.log('- Each returned a working SecureVault')
        console.log('- Balance queries work across all vaults')
