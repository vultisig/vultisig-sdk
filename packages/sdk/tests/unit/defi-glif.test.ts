...
describe('sdk.defi.glif — buildGlifStakeIcnt', () => {
  it('builds [approve, deposit] when allowance is insufficient and decodes to the right args', () => {
    const res = buildGlifStakeIcnt({ from: FROM, assetAmount: ONE })
    ...
    // approve(pool, assetAmount) on the ICNT token
    ...
    // deposit(assetAmount, receiver=from) on the pool
  ...
  it('drops the approve step when allowance already covers the assetAmount', () => {
    const res = buildGlifStakeIcnt({ from: FROM, assetAmount: ONE, currentAllowance: ONE })
  ...
  it('honors an INJECTABLE receiver (defaults to from, never hardcoded)', () => {
    const res = buildGlifStakeIcnt({ from: FROM, assetAmount: ONE, receiver: RECEIVER, currentAllowance: ONE })
  ...
  it('rejects non-positive, overflowing, and invalid inputs', () => {
    expect(() => buildGlifStakeIcnt({ from: FROM, assetAmount: 0n })).toThrow(/positive/)
    expect(() => buildGlifStakeIcnt({ from: FROM, assetAmount: -1n })).toThrow(/positive/)
    expect(() => buildGlifStakeIcnt({ from: FROM, assetAmount: 1n << 256n })).toThrow(/overflows uint256/)
    expect(() => buildGlifStakeIcnt({ from: 'not-an-address', assetAmount: ONE })).toThrow(/invalid address/)
  })
})

describe('sdk.defi.glif — buildGlifRedeemSticnt', () => {
  it('builds a single redeem(shares, receiver, owner) tx with owner pinned to from', () => {
    const res = buildGlifRedeemSticnt({ from: FROM, shareAmount: ONE })
  ...
  it('routes redeemed ICNT to an injected receiver while keeping owner = from', () => {
    const res = buildGlifRedeemSticnt({ from: FROM, shareAmount: ONE, receiver: RECEIVER })
    const decoded = decodeFunctionData({ abi: glifPoolWriteAbi, data: res.transactions[0].data })
  ...
  it('rejects bad amounts', () => {
    expect(() => buildGlifRedeemSticnt({ from: FROM, shareAmount: 0n })).toThrow(/positive/)
    expect(() => buildGlifRedeemSticnt({ from: FROM, shareAmount: 1n << 256n })).toThrow(/overflows uint256/)
    expect(() => buildGlifRedeemSticnt({ from: 'nope', shareAmount: ONE })).toThrow(/invalid address/)
  })
})
...
describe('sdk.defi.glif — fund-safety invariants (encoding == reported)', () => {
  ...
  it('stake: encoded deposit assetAmount == reported assetAmount, receiver == reported receiver', () => {
    const assetAmount = 123456789012345678901n // arbitrary non-round 18-decimal value
    const res = buildGlifStakeIcnt({ from: FROM, assetAmount, currentAllowance: assetAmount })
    expect(res.assetAmount).toBe(assetAmount)
    const decoded = decodeFunctionData({ abi: glifPoolWriteAbi, data: res.transactions[0].data })
    expect(decoded.functionName).toBe('deposit')
    expect(decoded.args[0]).toBe(assetAmount) // exact uint256, no truncation/wrap
    expect(getAddress(decoded.args[1] as string)).toBe(res.receiver)
  })

  it('stake: approve assetAmount == deposit assetAmount (no allowance/deposit drift)', () => {
    const assetAmount = 7n * ONE + 1n
    const res = buildGlifStakeIcnt({ from: FROM, assetAmount })
    const approve = decodeFunctionData({ abi: erc20Abi, data: res.transactions[0].data })
    const deposit = decodeFunctionData({ abi: glifPoolWriteAbi, data: res.transactions[1].data })
    expect(approve.functionName).toBe('approve')
    expect(approve.args[1]).toBe(assetAmount)
    expect(deposit.args[0]).toBe(assetAmount)
  ...
  it('redeem: encoded shares == reported shareAmount, owner pinned to from regardless of receiver', () => {
    const shareAmount = 98765432109876543210n
    const res = buildGlifRedeemSticnt({ from: FROM, shareAmount, receiver: RECEIVER })
    expect(res.shareAmount).toBe(shareAmount)
    const decoded = decodeFunctionData({ abi: glifPoolWriteAbi, data: res.transactions[0].data })
    expect(decoded.args[0]).toBe(shareAmount)
  ...
  it('selectors are pinned (deposit/redeem/approve) — wrong-selector regression guard', () => {
    const stake = buildGlifStakeIcnt({ from: FROM, assetAmount: ONE })
    ...
    const redeem = buildGlifRedeemSticnt({ from: FROM, shareAmount: ONE })
