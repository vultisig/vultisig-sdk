...
describe('RN entry wires configureCrypto and configureDefaultStorage', () => {
  ...
  it('re-exports root swap helpers needed by React Native consumers', () => {
    ...
    expect(typeof sdkRn.isTransientSwapQuoteError).toBe('function')
    expect(sdkRn.isTransientSwapQuoteError(new Error('ETIMEDOUT'))).toBe(true)
    expect(sdkRn.isTransientSwapQuoteError('no swap routes found: HTTP 503')).toBe(false)
