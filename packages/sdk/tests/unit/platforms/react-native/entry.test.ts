...
describe('RN entry exposes fromChainAmountExact + getBlockExplorerUrl', () => {
  ...
  it('re-exports the recent pure parse/normalize/decode helpers from the RN entrypoint', async () => {
    ...
    expect(rn.deriveToolOutputCandidateResult).toBe(tx.deriveToolOutputCandidateResult)
    expect(rn.buildTxReadyFromToolOutput).toBe(tx.buildTxReadyFromToolOutput)
    expect(rn.buildTxReadyFromYieldOutput).toBe(tx.buildTxReadyFromYieldOutput)
    expect(rn.SIGNABLE_FLAT_TOOLS).toBe(tx.SIGNABLE_FLAT_TOOLS)
    expect(rn.SIGNABLE_PREP_TOOLS).toBe(tx.SIGNABLE_PREP_TOOLS)
    expect(rn.SIGNABLE_YIELD_TOOLS).toBe(tx.SIGNABLE_YIELD_TOOLS)
