...
describe('Server Endpoints Tests', () => {
  ...
  describe('FastVault Endpoints', () => {
    it('GET /get/{public_key_ecdsa} - works with correct fetch implementation', async () => {
      ...
      try {
        ...
        console.log(
          '✅ Function works perfectly with proper fetch implementation'
        )
      } finally {
    ...
    it('POST /vault/sign - successfully initiates keysign process', async () => {
      ...
      const hexEncryptionKey = Array.from(
      ...
      const params = {
        ...
        messages: [
          'deadbeef01234567890abcdef01234567890abcdef01234567890abcdef012345',
        ],
        session: sessionId,
  ...
  describe('MessageRelay Endpoints', () => {
    ...
    it('Start/Complete Endpoints - work as expected', async () => {
      ...
      const completeKeysign = await fetch(
        `https://api.vultisig.com/router/complete/${sessionId}/keysign`,
        { method: 'POST' }
      )
      expect(completeKeysign.status).toBe(404)
      console.log(
        '✅ POST /complete/{sessionId}/keysign → 404 (expected for non-existent operation)'
      )

      const getCompleteKeysign = await fetch(
        `https://api.vultisig.com/router/complete/${sessionId}/keysign`
      )
      expect(getCompleteKeysign.status).toBe(404)
      console.log(
        '✅ GET /complete/{sessionId}/keysign → 404 (expected for non-existent operation)'
      )
    })
    ...
    it('Message Operations - all work correctly', async () => {
      ...
      const upload = await fetch(
        `https://api.vultisig.com/router/message/${sessionId}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      console.log(
        '✅ POST /message/{sessionId} → Message uploaded successfully'
      )
      ...
      console.log(
        '✅ GET /message/{sessionId}/{participantId} → Messages retrieved:',
        messages.length,
        'messages'
      )

      // DELETE /message/{sessionId}/{participantId}/{hash} - Delete message
      const deleteMsg = await fetch(
        `https://api.vultisig.com/router/message/${sessionId}/${participantId}/abcd1234`,
        {
          method: 'DELETE',
        }
      )
      ...
      console.log(
        '✅ DELETE /message/{sessionId}/{participantId}/{hash} → Message deleted successfully'
      )
    })
    ...
    it('Payload Operations - work correctly', async () => {
      ...
      const store = await fetch(
        `https://api.vultisig.com/router/payload/${testHash}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (store.status === 200) {
        ...
        console.log(
          'ℹ️ Payload endpoints not implemented (404) - this is acceptable'
        )
