...
describe('Signing Flow Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const vaultFile = new File([vaultBytes], 'TestFastVault.vult', {
  ...
  it('validates vault is accessible on server (fast vault check)', async () => {
    ...
    console.log(
      '✅ Vault successfully retrieved from server - this is a fast vault!'
    )
  ...
  it('validates vault has required signers for fast signing', async () => {
    ...
    const hasServerSigner = vaultData.signers.some((signer: string) =>
    ...
    if (!hasServerSigner) {
      console.log(
        '⚠️ This vault does not have a VultiServer signer - fast signing not available'
      )
      console.log(
        '   This test will validate the error handling for non-fast vaults'
      )
    } else {
  ...
  async function verifySignature(
    ...
    const { serializeTransaction, keccak256, recoverAddress } = await import(
      'viem'
    )
    ...
    if (signature.format === 'ECDSA' && signature.recovery !== undefined) {
      ...
      console.log(
        '   Expected address: ',
        vaultData.addresses?.Ethereum || 'Not available'
      )
      ...
      console.log(
        '   The signature is cryptographically valid for our transaction!'
      )
    } else {
      console.log(
        '⚠️  Signature verification requires ECDSA format with recovery ID'
      )
    }
  }
  ...
  it('validates server session management in signing flow', async () => {
    ...
    try {
      ...
      const sessionId = await signWithServer({
        ...
        messages: [
          'c0a27b8a8926f38c54daac8da682113ed78d51e35271472153538383d7ee0646',
        ], // ETH tx hash
        session: crypto.randomUUID(),
        hex_encryption_key: Array.from(
      ...
      try {
        const startResponse1 = await globalThis.fetch(
          `https://api.vultisig.com/router/start/${sessionId}`,
          {
            method: 'POST',
          }
        )
        console.log(
          `   Method 1 (/start/{sessionId}): ${startResponse1.status}`
        )
      } catch (error) {
      ...
      try {
        const startResponse2 = await globalThis.fetch(
          `https://api.vultisig.com/router/${sessionId}`,
          {
            method: 'POST',
        ...
        console.log(
          `   Method 2 (/{sessionId} with server): ${startResponse2.status}`
        )
      } catch (error) {
      ...
      try {
        const startResponse3 = await globalThis.fetch(
          `https://api.vultisig.com/router/${sessionId}/start`,
          {
            method: 'POST',
          }
        )
        console.log(
          `   Method 3 (/{sessionId}/start): ${startResponse3.status}`
        )
      } catch (error) {
      ...
      while (!serverJoined && attempts < maxAttempts) {
        ...
        if (hasServer && hasClient) {
          ...
          console.log(
            `   Waiting... (Server: ${hasServer}, Client: ${hasClient})`
          )
      ...
      if (!serverJoined) {
        ...
        console.log(
          '   This is expected in test environment - server MPC participation requires full infrastructure'
        )
        return
      }
      ...
      const startTssResponse = await globalThis.fetch(
        `https://api.vultisig.com/router/start/${sessionId}`,
        {
          method: 'POST',
        }
      ...
      while (!signingComplete && messageAttempts < maxMessageAttempts) {
        ...
        const messagesResponse = await globalThis.fetch(
          `https://api.vultisig.com/router/message/${sessionId}/${vaultData.localPartyId}`
        )

        if (messagesResponse.ok) {
          const messages = await messagesResponse.json()
          console.log(
            `   Message attempt ${messageAttempts + 1}: Received ${messages.length} messages`
          )

          if (messages.length > 0) {
            console.log(
              '   📨 Received MPC messages from server:',
              messages.length
            )
        ...
        const keysignResponse = await globalThis.fetch(
          `https://api.vultisig.com/router/complete/${sessionId}/keysign`
        )
      ...
      if (!signingComplete) {
        ...
        console.log(
          '   This requires full MPC library integration and server-side signing participation'
        )
      }
