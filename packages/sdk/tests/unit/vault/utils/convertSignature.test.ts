...
describe("convertToKeysignSignatures", () => {
  ...
  describe("Error handling", () => {
    it("should throw error when message hash is missing for single signature", () => {
      ...
      expect(() =>
        convertToKeysignSignatures(signature, messageHashes),
      ).toThrow("No message hash provided for signature");
    });
    ...
    it("should throw error when message hash is missing for multi-signature", () => {
      ...
      const messageHashes = [
        "0x1111111111111111111111111111111111111111111111111111111111111111",
      ]; // Only 1 hash but 2 signatures

      expect(() =>
        convertToKeysignSignatures(signature, messageHashes),
      ).toThrow("Missing message hash for signature at index 1");
    });
  });
});
