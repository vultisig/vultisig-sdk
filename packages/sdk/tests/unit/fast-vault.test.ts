...
describe('Fast Vault Creation - Server API Integration', () => {
  ...
  it('should test fast vault API call structure', async () => {
    ...
    try {
      ...
      if (error?.message?.includes('fetch failed')) {
        console.log(
          'ℹ️ This is expected if servers are not available in test environment'
        )
      } else {
  ...
  it('should validate complete vault creation flow structure', async () => {
    ...
    console.log(
      '   1. POST /vault/create - ✅ Available via setupVaultWithServer'
    )
    console.log('   2. Join MPC session - ✅ Available via joinMpcSession')
    console.log(
      '   3. MPC keygen (ECDSA + EdDSA) - ✅ Available via DKLS & Schnorr classes'
    )
    console.log(
      '   4. Return complete vault - ✅ Returns vault with keys & shares'
    )
  ...
  it('should test with environment variables', async () => {
    ...
    console.log(
      ...
      vaultPassword ? '***set***' : 'Using default'
    )
    ...
    if (fastEmail) {
      ...
      console.log(
        `   Password: ${testParams.password ? '***set***' : 'not set'}`
      )
      ...
      console.log(
        '   Copy src/tests/signing/env.example to src/tests/signing/.env'
      )
  ...
  it('should create real fast vault with MPC keygen and wait for verification', async () => {
    ...
    if (!fastEmail) {
      console.log(
        '⚠️ Skipping full vault creation test - FAST_EMAIL not configured'
      )
      console.log(
        '   Set FAST_EMAIL in src/tests/signing/.env to run this test'
      )
      return
    }
    ...
    try {
      ...
      console.log(
        `   await serverManager.verifyVault('${result.vaultId}', 'YOUR_CODE')`
      )
      console.log('')
      console.log('📝 To retrieve the vault later, run:')
      console.log(
        `   await serverManager.getVaultFromServer('${result.vaultId}', '${vaultPassword}')`
      )
      console.log('')
      ...
      if (error?.message?.includes('fetch failed')) {
        ...
        console.error(
          '🔧 This might be a WASM initialization issue in test environment'
        )
      }
  ...
  it('should verify vault with email code (manual test)', async () => {
    ...
    console.log(
      '   3. Set VAULT_ID and VERIFICATION_CODE environment variables'
    )
    ...
    if (!vaultId || !verificationCode) {
      ...
      console.log(
        '   Set VAULT_ID and VERIFICATION_CODE in .env to run this test'
      )
      console.log('')
      console.log('💡 Example:')
      ...
      if (verified) {
        ...
        if (process.env.VAULT_PASSWORD) {
          ...
          const retrievedVault = await withWorkingFetch(async () => {
            return await serverManager.getVaultFromServer(
              vaultId,
