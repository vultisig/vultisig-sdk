...
vi.mock("@vultisig/core-mpc/devices/localPartyId", () => ({
  generateLocalPartyId: vi.fn(
    (prefix: string) =>
      `${prefix}-party-${Math.random().toString(36).slice(2, 8)}`,
  ),
}));
...
describe("SecureVaultCreationService", () => {
  ...
  describe("constructor", () => {
    ...
    it("should accept custom relay URL", () => {
      const customService = new SecureVaultCreationService(
        "https://custom.relay.com",
      );
  ...
  describe("SecureVaultCreationStep type", () => {
    it("should have valid step values", () => {
      ...
      const step: Parameters<
        NonNullable<Parameters<typeof service.createVault>[0]["onProgress"]>
      >[0] = {
        step: "initializing",
        progress: 0,
        message: "Test",
