...
describe("SwapService", () => {
  ...
  let mockEmitEvent: <K extends keyof VaultEvents>(
    event: K,
    data: VaultEvents[K],
  ) => void;
  ...
  describe("getQuote", () => {
    ...
    it("should fetch a swap quote for ERC-20 token requiring approval", async () => {
      ...
      expect(result.approvalInfo?.spender).toBe(
    ...
    it("should handle quote errors gracefully", async () => {
      ...
      vi.mocked(findSwapQuote).mockRejectedValue(
        new Error("No swap routes found"),
      );
  ...
  describe("prepareSwapTx", () => {
    it("should prepare swap transaction", async () => {
      const { buildSwapKeysignPayload } = await import(
        "@core/mpc/keysign/swap/build"
      );
      ...
      vi.mocked(buildSwapKeysignPayload).mockResolvedValue(
        mockKeysignPayload as any,
      );
  ...
  describe("isSwapSupported", () => {
    it("should return true for supported chain pairs", () => {
      ...
      expect(service.isSwapSupported(Chain.Ethereum, Chain.Ethereum)).toBe(
        true,
      );
