...
describe("fiatToAmount", () => {
  ...
  it("converts USD to native token amount using market price", async () => {
    const { getCoinPrices } =
      await import("@vultisig/core-chain/coin/price/getCoinPrices");
  ...
  it("uses ERC-20 price helper when tokenId is provided on EVM chain", async () => {
    const { getErc20Prices } =
      await import("@vultisig/core-chain/coin/price/evm/getErc20Prices");
  ...
  it("throws on invalid (non-positive) fiatValue", async () => {
    await expect(
      fiatToAmount({ fiatValue: 0, chain: Chain.Ethereum, decimals: 18 }),
    ).rejects.toThrow(/fiat value/i);

    await expect(
      fiatToAmount({ fiatValue: -10, chain: Chain.Ethereum, decimals: 18 }),
    ).rejects.toThrow(/fiat value/i);

    await expect(
      fiatToAmount({ fiatValue: "abc", chain: Chain.Ethereum, decimals: 18 }),
    ).rejects.toThrow(/fiat value/i);
  });
