...
export class SwapService {
  constructor(
    ...
    private emitEvent: <K extends keyof VaultEvents>(
      event: K,
      data: VaultEvents[K],
    ) => void,
  ) {}
  ...
  async getQuote(params: SwapQuoteParams): Promise<SwapQuoteResult> {
    try {
      ...
      const approvalInfo = await this.checkApprovalRequired(
        fromCoin,
        params.amount,
        quote,
      );
      ...
      // Format the result
      const result = this.formatQuoteResult(
        quote,
        fromCoin,
        toCoin,
        approvalInfo,
    ...
    try {
      ...
      if (Date.now() > params.swapQuote.expiresAt) {
        throw new VaultError(
          ...
          "Swap quote has expired. Please refresh the quote.",
        );
      }
      ...
      if (!params.autoApprove && keysignPayload.erc20ApprovePayload) {
        ...
        this.emitEvent("swapApprovalRequired", {
          ...
          currentAllowance:
            params.swapQuote.approvalInfo?.currentAllowance.toString() ?? "0",
        });
  ...
  private formatQuoteResult(
    ...
    // Calculate expiry
    const expiresIn = isNative
      ? quote.native.expiry * 1000 - Date.now()
    ...
    const provider = isNative
      ? quote.native.swapChain.toLowerCase()
      : quote.general.provider;
  ...
  private wrapSwapError(error: unknown): VaultError {
    ...
    if (
      message.includes("No swap routes") ||
      message.includes("NoSwapRoutesError")
    ) {
    ...
    if (
      ...
      message.includes("not enough asset")
    ) {
      return new VaultError(
        VaultErrorCode.InvalidConfig,
