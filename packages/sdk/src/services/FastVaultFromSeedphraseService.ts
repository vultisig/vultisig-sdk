...
export class FastVaultFromSeedphraseService {
  ...
  async createFromSeedphrase(
    ...
    if (options.discoverChains) {
      ...
      const discoveryResult = await this.discoveryService.discoverChains(
        mnemonic,
        {
    ...
    const chainsToImport =
      options.chains ??
      discoveredChains?.filter((c) => c.hasBalance).map((c) => c.chain) ??
      DEFAULT_CHAINS;
    ...
    try {
      ...
      console.warn(
        "ML-DSA keygen failed (non-fatal), vault will be created without post-quantum keys:",
  ...
  private async waitForPeers(
    ...
    throw new VaultError(
      ...
      "Timeout waiting for server to join key import session",
    );
  }
}
