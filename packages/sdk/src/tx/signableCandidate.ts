/**
 * Pure tool-output to signable-candidate derivation.
 *
 ...
 *
 * SDK-owned, vault-free, side-effect-free, and shared by first-party clients.
 */
...
/** Signer-ready envelope produced by the pure derivation helpers. */
export type SignableToolOutputPayload = Record<string, unknown> & {
  chain?: string
  chain_id?: string
  action?: string
...
const resolveChain = (value: string): Chain | null => resolveChainReference(value) ?? null
const resolveChainId = (value: string | number): Chain | null => resolveChainReference(value) ?? null
 ...
 */
export const SIGNABLE_FLAT_TOOLS: ReadonlySet<string> = new Set([
 ...
 */
export const SIGNABLE_PREP_TOOLS: ReadonlySet<string> = new Set([
  'execute_swap',
  'execute_send',
 ...
 */
export const SIGNABLE_YIELD_TOOLS: ReadonlySet<string> = new Set(['yield_enter', 'yield_exit'])
 ...
 */
export function buildTxReadyFromToolOutput(toolName: string, output: unknown): SignableToolOutputPayload | null {
  if (!SIGNABLE_FLAT_TOOLS.has(toolName)) return null
 ...
 */
export function buildTxReadyFromYieldOutput(toolName: string, output: unknown): SignableToolOutputPayload | null {
  if (!SIGNABLE_YIELD_TOOLS.has(toolName)) return null
...
export type ToolOutputCandidate = {
  payload: SignableToolOutputPayload
  source: 'flat' | 'prep' | 'yield'
  toolName: string
}

export type ToolOutputCandidateFailureReason = 'unsupported-tool' | 'unsafe-or-malformed-output'

export type ToolOutputCandidateResult =
  | { candidate: ToolOutputCandidate; reason?: never }
  | { candidate: null; reason: ToolOutputCandidateFailureReason }
...
export function deriveToolOutputCandidate(toolName: string, output: unknown): ToolOutputCandidate | null {
  ...
  if (SIGNABLE_PREP_TOOLS.has(toolName)) {
    ...
    return { payload: env as SignableToolOutputPayload, source: 'prep', toolName }
  }
  return null
}

/**
 * Derive a signable candidate while retaining an explicit fail-closed reason.
 *
 * The nullable helper above remains convenient for signing gates. Consumers
 * that need telemetry or user-neutral diagnostics can use this result form
 * without inspecting or logging the potentially sensitive raw tool output.
 */
export function deriveToolOutputCandidateResult(toolName: string, output: unknown): ToolOutputCandidateResult {
  const candidate = deriveToolOutputCandidate(toolName, output)
  if (candidate) return { candidate }

  const supported =
    SIGNABLE_FLAT_TOOLS.has(toolName) || SIGNABLE_PREP_TOOLS.has(toolName) || SIGNABLE_YIELD_TOOLS.has(toolName)
  return { candidate: null, reason: supported ? 'unsafe-or-malformed-output' : 'unsupported-tool' }
}
