 ...
 */
export const suiTransactionDataIntent = new Uint8Array([0, 0, 0])
export const suiPersonalMessageIntent = new Uint8Array([3, 0, 0])
 ...
 */
export const getSuiTransactionDataDigest = (
  txBytes: Uint8Array
): Uint8Array => blake2b(
  concatBytes(suiTransactionDataIntent, txBytes),
  { dkLen: 32 }
)
 ...
 * hashing.
 */
export const getSuiPersonalMessageDigest = (
  messageBytes: Uint8Array
): Uint8Array => blake2b(
  concatBytes(
    suiPersonalMessageIntent,
    encodeUleb128(messageBytes.length),
    messageBytes
  ),
  { dkLen: 32 }
)
 ...
 */
export const buildSuiSerializedSignature = ({
  signature,
  publicKey,
}: BuildSuiSerializedSignatureInput): Uint8Array => {
  if (signature.length !== ed25519SignatureLength) {
    throw new Error(
      `Sui Ed25519 signature must be ${ed25519SignatureLength} bytes, got ${signature.length}`
    )
  }
  if (publicKey.length !== ed25519PublicKeyLength) {
    throw new Error(
      `Sui Ed25519 public key must be ${ed25519PublicKeyLength} bytes, got ${publicKey.length}`
    )
  }
  const out = new Uint8Array(
    1 + ed25519SignatureLength + ed25519PublicKeyLength
  )
