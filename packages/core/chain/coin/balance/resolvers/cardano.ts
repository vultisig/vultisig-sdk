...
export const getCardanoCoinBalance: CoinBalanceResolver<OtherChain.Cardano> = async input => {
  if (input.id) {
    ...
    const match = assets.find(
      a => a.policy_id.toLowerCase() === normalizedPolicyId && a.asset_name.toLowerCase() === normalizedAssetName
    )
  ...
  const [{ balance } = { balance: '0' }] = await queryUrl<CardanoAddressInfoResponse>(url, {
    body: {
