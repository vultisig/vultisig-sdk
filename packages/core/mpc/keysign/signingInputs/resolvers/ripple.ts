  ...
  const getPayment = (): Pick<TW.Ripple.Proto.ISigningInput, 'opPayment' | 'rawJson'> => {
    ...
    if (distinctMemo) {
      ...
      const txJson = {
        ...
        // Unlike the OperationPayment PROTO path (int64 field, so signedness
        // must match), this is a plain JSON string: XRP drops are non-negative,
        // so reject a negative amount here instead of building JSON that XRPL
