...
const makeRippleSpecific = (destinationTag?: number) => {
  ...
  if (destinationTag !== undefined)
    rippleSpecific.destinationTag = destinationTag;
...
describe("getRippleSigningInputs -- TrustSet build path (issued currency)", () => {
  ...
  it.each(["001", "4294967296"])(
    "keeps non-canonical legacy numeric memo %s as a memo",
    async (memo) => {
      const [input] = await getRippleSigningInputs({
  ...
  it.each([-1, 4294967296, 1.5])(
    "rejects an invalid first-class destination tag: %s",
    (destinationTag) => {
      expect(() =>
...
describe("getRippleSigningInputs -- rawJson build path (dApp-supplied tx)", () => {
  ...
  it("throws on an empty rawJson instead of emitting an operation-less input", () => {
    ...
    expect(() =>
      getRippleSigningInputs({ keysignPayload: payload, walletCore }),
    ).toThrow(/missing rawJson/);
  });
...
const vector = JSON.parse(
  readFileSync(
    new URL("../fixtures/ripple-interop-vector.json", import.meta.url),
    "utf8",
  ),
...
describe("getRippleSigningInputs interop vector", () => {
  ...
  it("pins destination-tag payment fields for the shared vector", async () => {
    ...
    expect(input.opPayment?.destinationTag?.toString()).toBe(
      vector.input.destinationTag,
    );
    expect(input.sequence).toBe(Number(vector.input.sequence));
    expect(input.fee.toString()).toBe(vector.input.fee);
    expect(input.lastLedgerSequence).toBe(
      Number(vector.input.lastLedgerSequence),
    );
  });
