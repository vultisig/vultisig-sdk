...
const shortMemo = 'm=<:100000000BTC.BTC:5000000THOR.RUNE:0'
...
describe('buildLimitSwapCancelKeysignPayload', () => {
  ...
  describe('L1 source (dust transfer to the inbound vault)', () => {
    it('targets the live Asgard vault and attaches derived dust', async () => {
      const payload = await buildLimitSwapCancelKeysignPayload({ ...baseInput, signingCoin: ethCoin })
    ...
    it('refuses a halted source chain rather than paying into a dead vault', async () => {
      ...
      await expect(buildLimitSwapCancelKeysignPayload({ ...baseInput, signingCoin: ethCoin })).rejects.toThrow(
    ...
    it('refines the payload for a UTXO source', async () => {
      await buildLimitSwapCancelKeysignPayload({ ...baseInput, memo: shortMemo, signingCoin: btcCoin })
  ...
  describe('fail-closed gates', () => {
    ...
    it('refuses a memo that overflows a UTXO source OP_RETURN', async () => {
      await expect(buildLimitSwapCancelKeysignPayload({ ...baseInput, signingCoin: btcCoin })).rejects.toThrow(
        /memo budget/
      )
    })
