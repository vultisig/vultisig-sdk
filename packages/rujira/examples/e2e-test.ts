...
import { ASSETS, EASY_ROUTES, RujiraClient } from '../src'
...
async function main() {
  ...
  try {
    ...
    const addrMaybe = typeof addrFn === 'function' ? addrFn.call(vault, 'Ethereum') || addrFn.call(vault, 'ETH') : ''
    const addrFromVault = addrMaybe instanceof Promise ? await addrMaybe : addrMaybe

    const ethCoin = coins.find(
      (c: any) => (c?.chain || '').toLowerCase() === 'ethereum' || (c?.ticker || '').toLowerCase() === 'eth'
    )
