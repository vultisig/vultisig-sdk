...
describe('RujiraDeposit', () => {
  ...
  describe('buildDepositMemo', () => {
    ...
    it('should build deposit memo with affiliate', () => {
      const memo = deposit.buildDepositMemo(VALID_THOR_ADDRESS, 'thor1affiliate', 50)
    ...
    it('should not include affiliate if bps is 0', () => {
      const memo = deposit.buildDepositMemo(VALID_THOR_ADDRESS, 'thor1affiliate', 0)
      expect(memo).toBe(`secure+:${VALID_THOR_ADDRESS}`)
    })

    it('should reject addresses with colon (injection prevention)', () => {
      expect(() => deposit.buildDepositMemo('thor1abc:evil')).toThrow("contains ':'")
    })

    it('should reject affiliate with colon (injection prevention)', () => {
      expect(() => deposit.buildDepositMemo(VALID_THOR_ADDRESS, 'aff:evil', 50)).toThrow("contains ':'")
    })
  })
  ...
  describe('getBalance', () => {
    it('should fetch balance for specific asset', async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            balances: [{ denom: 'btc-btc', amount: '100000000' }],
          }),
      })
  ...
  describe('getInboundAddress', () => {
    ...
    it('should be case insensitive', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve([{ chain: 'BTC', address: 'bc1q...', halted: false }]),
      })
