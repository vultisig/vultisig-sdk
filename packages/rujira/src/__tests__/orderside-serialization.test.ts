...
import { type ContractSide, fromContractSide, type OrderSide, toContractSide } from '../types.js'
...
describe.skipIf(!process.env.LIVE_TESTS)('Live OrderSide Verification', () => {
  ...
  it('verifies contract book response uses base/quote format', async () => {
    ...
    const contractsRes = await fetch(`${THORNODE_URL}/cosmwasm/wasm/v1/code/73/contracts?pagination.limit=1`)
    ...
    const bookRes = await fetch(`${THORNODE_URL}/cosmwasm/wasm/v1/contract/${contractAddr}/smart/${bookQuery}`)
  ...
  it('verifies contract config can be queried', async () => {
    ...
    const configRes = await fetch(`${THORNODE_URL}/cosmwasm/wasm/v1/contract/${contractAddr}/smart/${configQuery}`)
