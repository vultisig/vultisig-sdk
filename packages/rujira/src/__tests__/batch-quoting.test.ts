...
const createMockClient = (failingContractPatterns: string[] = []) => {
  ...
  return {
    ...
    simulateSwap: vi.fn().mockImplementation(async (contract: string, _denom: string) => {
...
describe('Batch Quoting', () => {
  describe('batchGetQuotes()', () => {
    ...
    it('should execute quotes in parallel', async () => {
      ...
      await swap.batchGetQuotes(['RUNE_TO_USDC', 'RUNE_TO_BTC', 'RUNE_TO_ETH'], '100000000')
    ...
    it('should pass destination to quotes', async () => {
      ...
      await swap.batchGetQuotes(['RUNE_TO_USDC'], '100000000', 'thor1qperwt9wrnkg5k9e5gzfgjppxgnwqdwqgglcvrl')
