export function getDiscriminatedUnionValue<
  ...
  T extends U[D],
>(unionValue: U, discriminantKey: D, valueKey: V, expectedCase: T): Extract<U, { [P in D]: T }>[V] {
  if (unionValue[discriminantKey] !== expectedCase) {
    throw new Error(`Expected case "${expectedCase}", but got "${unionValue[discriminantKey]}".`)
  }
