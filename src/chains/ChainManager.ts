...
export class ChainManager {
  ...
  async getAddresses(
    ...
    for (const chain of chains) {
      ...
      addresses[chain] = await this.addressDeriver.deriveAddress(
        vault,
        chainStr
      )
    }
  ...
  async getAddressesByKind(
    ...
    for (const chainKind of chainKinds) {
      ...
      if (primaryChain) {
        ...
        addresses[chainKind] = await this.addressDeriver.deriveAddress(
          vault,
          chainStr
        )
      }
  ...
  async getBalancesByKind(
    ...
    for (const [chainKind, address] of Object.entries(addresses)) {
      try {
        ...
        if (primaryChain) {
          balances[chainKind as ChainKind] = await this.getChainBalance(
            primaryChain,
            address
          )
        }
  ...
  getChainClient(chain: Chain) {
    ...
    throw new Error(
      'getChainClient not implemented yet - requires core chain client integration'
    )
  }
