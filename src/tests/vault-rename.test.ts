...
describe('Vault Rename Tests', () => {
  ...
  describe('Basic Rename Operations', () => {
    ...
    test('should update vault in storage after rename', async () => {
      ...
      const renamedVault = vaultList.find(v => v.id === vault.data.publicKeys.ecdsa)
  ...
  describe('Name Validation', () => {
    ...
    test('should reject name with invalid characters', async () => {
      ...
      await expect(vault.rename('Name with $ symbols')).rejects.toThrow(VaultError)
      await expect(vault.rename('Name with % percent')).rejects.toThrow(VaultError)
    })
  ...
  describe('Error Handling', () => {
    test('should throw VaultError with InvalidConfig code for validation errors', async () => {
      ...
      try {
        ...
        expect((error as VaultError).message).toContain('Vault name cannot be empty')
      }
    })
    ...
    test('should provide specific error messages for different validation failures', async () => {
      ...
      try {
        ...
        expect((error as VaultError).message).toContain('Vault name cannot exceed 50 characters')
      }
      ...
      try {
        ...
        expect((error as VaultError).message).toContain('Vault name can only contain letters, numbers, spaces, hyphens, and underscores')
      }
    })
  })
  ...
  describe('Multiple Vaults', () => {
    test('should rename specific vault without affecting others', async () => {
      ...
      const renamedVault = vaultList.find(v => v.id === vault1.data.publicKeys.ecdsa)
      const unchangedVault = vaultList.find(v => v.id === vault2.data.publicKeys.ecdsa)
