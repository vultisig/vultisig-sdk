...
|----------|-----------|-----|
| **AI agent chat** | [CLI `agent ask`](./vultisig-cli/SKILL.md#agent-chat-ai-to-ai) | One-shot natural language crypto ops; use `--session` for multi-turn, `--password` for signing |
...
**Use Agent Ask if**: You're an AI coding agent (Claude Code, Opencode, Cursor) that needs to perform crypto operations via natural language. Each invocation is one-shot; pass `--session <id>` to continue a prior conversation. Signing operations require `--password`.
